...
base <- leaflet() %>%
  ...
  setView(lng = lon, lat = lat, zoom = 6.4)
...
<!-- 
```{ojs}
// Fetch the GeoJSON data (replace with your actual paths)
wa_counties = FileAttachment("data/wa_counties.geojson").json()
bigfoot_points = FileAttachment("data/bigfoot_prints.geojson").json()

// Create a custom icon for the markers (replace with your custom icon)
BigFootIcon = icon({ 
  iconUrl: "https://images.fineartamerica.com/images/artworkimages/medium/3/gluten-free-cute-bigfoot-cartoon-noirty-designs-transparent.png",  // Replace with the URL to your custom icon
  iconSize: [32, 32],  // Adjust the size as necessary
});

// Create the map
viewof map = map()
  .setView([47.7511, -120.7401], 7)  // Center the map on Washington State
//   .addLayer(tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png'))  // Add base OpenStreetMap tiles

  // Add county outlines as polylines
  .addLayer(geoJSON(wa_counties).setStyle({color: '#595959'}))  // Set color for counties outline

  // Add Bigfoot markers (points) with popups and custom icons
  .addLayer(
    geoJSON(bigfoot_points, {
      pointToLayer: function (feature, latlng) {
        return marker(latlng, {icon: BigFootIcon});
      },
      onEachFeature: function (feature, layer) {
        // Attach the popup and label
        layer.bindPopup(feature.properties.popup);
        layer.bindTooltip(feature.properties.summary);
      }
    })
  );
